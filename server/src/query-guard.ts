// Kept in sync with electron/query-guard.cjs — the desktop app and the lab
// server must enforce the same SELECT rules.
...
type Tok = { t: "name" | "word" | "punct"; v: string };
...
function isIdent(tok: Tok | undefined): tok is Tok {
  return Boolean(tok && (tok.t === "name" || (tok.t === "word" && !NOT_A_TABLE.has(tok.v))));
}
 ...
 * `\bfrom\s+(\w+)` match, `SELECT * FROM ds_x, users` reported only `ds_x` and
 * the second table was never checked — any user with read on any project could
 * select users.password_hash and sessions.token_hash.
 */
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    for (;;) {
      ...
      if (cur.t === "punct" && cur.v === "(") {
        ...
        // table-valued function, e.g. pg_read_file('...')
...
/**
 * Names bound by a leading WITH clause. Postgres resolves these ahead of any
 * real table, so they are not project datasets and must not be checked against
 * the dataset list — before this, every CTE was rejected as "not part of this
 * project", making WITH queries unusable despite being explicitly allowed.
 */
 ...
 * the one way to smuggle DML into a SELECT on Postgres). Scanning raw SQL for
 * the bare word instead rejected ordinary data — a column named `comment`, an
 * alias named `copy`, a value like 'Update 2023'. The word must also be
 * followed by another token, so `upper(comment)` stays a column reference.
 */
function findBlockedKeyword(sql: string): string | null {
